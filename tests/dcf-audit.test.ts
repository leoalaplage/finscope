...
describe("the divisor is reported, not left to be inferred", () => {
  it("states valuation shares, and they exceed the projected ending count under a buyback", () => {
    const result = calculateDcf(base, assumptions({ shareChange: fill(-.03) }));
    const ending = result.projections.at(-1)!.dilutedShares;
    expect(result.valuationShares).toBeGreaterThan(ending);
    // The number on screen has to reproduce the number beside it.
    expect(result.equityValue! / result.valuationShares).toBeCloseTo(result.intrinsicValuePerShare!, 6);
  });

  it("uses the projected count when the assumption only issues shares", () => {
    const result = calculateDcf(base, assumptions({ shareChange: fill(.02) }));
    expect(result.valuationShares).toBeCloseTo(result.projections.at(-1)!.dilutedShares, 6);
  });
});
