...
**FAIT CONSTATÉ — le typage économique n'est plus déduit d'un libellé libre.** Les CIK vérifiés classent JPMorgan comme banque, Berkshire comme holding, Interactive Brokers comme courtier, CME et Cboe comme places de marché. Pour toute société résolue dynamiquement, le code SIC officiel de la SEC classe désormais automatiquement banques, courtiers, assureurs, holdings et autres activités financières avant tout calcul industriel. Le type historique générique `financial` reste accepté pour les profils déjà stockés dans un navigateur.
...
**FAIT CONSTATÉ — nouvelle frontière de cache.** Ces changements de sens ont imposé successivement v17 pour le fail-closed et les types vérifiés, v18 pour la lecture correcte de `DebtCurrent`, puis `company:v19:<ticker>` pour la classification SIC des sociétés dynamiques. Les digests passent à `summary:v19.s7:<ticker>` afin qu'un vieux TTM ne supplante plus un exercice annuel récent. Aucune version antérieure n'est autorisée comme repli pendant la reconstruction : v18 peut encore présenter une banque dynamique comme une société industrielle. `DebtCurrent` est un synonyme de la portion courante — emprunts à court terme et échéances courantes de la dette long terme réunis — et non une ligne additionnelle : une première version l'ajoutait à `LongTermDebtCurrent` et sortait NVIDIA à 9 467 M$ d'emprunts contre les 8 468 M$ que la société publie elle-même, et S&P Global à 13 806 contre 13 088. `docs/SOURCES.md` décrit les quatre crons, le budget de six reconstructions, le warm-on-read, le TTL KV de sept jours et le cache edge d'une heure.

**FAIT CONSTATÉ — contrôles locaux verts.** `npm test` passe sur 42 fichiers : 606 tests réussis et 1 ignoré. `npx tsc --noEmit`, `npm run lint`, `npm run build` et le smoke test de rendu serveur passent également. Les nouveaux cas couvrent la classification par CIK et SIC, le total de borrowings JPM/CME, le refus d'un composant de dette isolé, la preuve par montant brut de la fonction économique d'une ligne de dette ambiguë, le pont DCF sans zéro implicite, les mesures financières retirées du screener et des digests, l'ancrage d'un dépôt financier dépourvu de revenu industriel, les libellés TTM/fixture et le choix de la période réellement la plus récente.

**FAIT CONSTATÉ — contrôle live multi-secteurs.** L'adaptateur courant a été exécuté contre les Company Facts SEC de A, ADBE, AMZN, BAC, COST, CRM, GS, JPM, ORCL, PGR, TSLA et NKE. ADBE TTM 2026-05-29 reconstruit 6,645 Md$ de dette et un ROIC de 54,5 % ; BAC, GS, JPM et PGR sont correctement reconnus comme financiers et ne publient pas de ROIC industriel ; Goldman et JPM utilisent `RevenuesNetOfInterestExpense` comme revenu financier standardisé. NKE reste sans ROIC parce que son résultat opérationnel récent est une extension propre à la société, non portée par Company Facts standardisé.

### 22.2 Restant ouvert

- **À REVALIDER EN PRODUCTION.** Le rapprochement des sept sociétés de la section 13, plus Adobe, NVIDIA, S&P Global et une banque résolue dynamiquement, doit être rejoué après déploiement et reconstruction v19 ; le code et les tests locaux ne prouvent pas à eux seuls le contenu futur du KV de production.
...
- **TYPOLOGIE AUTOMATISÉE MAIS PRUDENTE.** Les cinq CIK vérifiés corrigent les cas où le SIC est trop large ; les autres sociétés dynamiques utilisent la classification SIC officielle de la SEC. Les codes financiers non spécifiques restent `financial` plutôt que d'être forcés dans une catégorie plus précise.
- **COUVERTURE UNIVERSELLE NON ATTEINTE.** Les extensions XBRL propres aux sociétés ne sont pas encore lues : NKE en est un cas actuel pour le résultat opérationnel. Les émetteurs américains déposant uniquement en IFRS, comme TSM, nécessitent un adaptateur `ifrs-full` distinct. Ces frontières sont explicites et fail-closed ; les présenter comme une compatibilité avec « n'importe quel titre US » serait faux tant que ces deux chantiers ne sont pas terminés.
