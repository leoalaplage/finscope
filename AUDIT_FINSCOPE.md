...
**FAIT CONSTATÉ — nouvelle frontière de cache.** Ces changements de sens imposent `company:v17:<ticker>` et `summary:v17.s6:<ticker>`. Aucune version antérieure n'est autorisée comme repli pendant la reconstruction. `docs/SOURCES.md` décrit maintenant les quatre crons, le budget de six reconstructions, le warm-on-read, le TTL KV de sept jours et le cache edge d'une heure.

**FAIT CONSTATÉ — contrôles locaux verts.** `npm test` passe sur 41 fichiers : 595 tests réussis et 1 ignoré. `npx tsc --noEmit`, `npm run lint`, `npm run build` et le smoke test de rendu serveur passent également. Les nouveaux cas couvrent la classification par CIK, le total de borrowings JPM/CME, le refus d'un composant de dette isolé, le pont DCF sans zéro implicite, les mesures financières retirées du screener et des digests, ainsi que les libellés TTM/fixture.

### 22.2 Restant ouvert

- **À REVALIDER EN PRODUCTION.** Le rapprochement des sept sociétés de la section 13 doit être rejoué après déploiement et reconstruction v17 ; le code et les tests locaux ne prouvent pas à eux seuls le contenu futur du KV de production.
