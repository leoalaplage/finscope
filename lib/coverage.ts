...
export function auditMetric(dataset: CompanyDataset, metric: string, periodicity: Periodicity): MetricAudit {
  ...
  if(balanceSheetIsTheBusiness(dataset.company.businessType)&&FINANCIAL_NOT_APPLICABLE.has(metric))return {metric,periodicity,status:"Not economically applicable",reason:"Free-cash-flow and industrial return-on-capital measures are withheld for financial institutions; customer and clearing balances make the arithmetic economically misleading.",firstPeriod:null,lastPeriod:null,available:0,total:periods.length,...empty};
