...
// Scoring
// ---------------------------------------------------------------------
function segment(valeur, a, b, sa, sb) {
  if (a === b) return sb;
  return sa + ((valeur - a) / (b - a)) * (sb - sa);
}

/**
 * Score absolu 0-100 d'une valeur brute, d'apres les ancres de config.
 *
 * L'echelle est affine par morceaux entre trois bornes — 0, 50, 100 — donc
...
function scoreAbsolu(cle, valeur) {
  ...
  const score = auDela(v50, valeur)
    ...
    : segment(valeur, v50, v100, 50.0, 100.0);
...
/**
 * score par metrique = MELANGE_ABSOLU * ancres + MELANGE_RELATIF * percentile.
 *
 * Les poids par defaut sont 1 et 0 : la note d'une societe se lit sur une
 * echelle fixe et ne bouge pas selon les titres notes en meme temps. Le
 * percentile ne sert plus que de filet quand une metrique n'a pas d'ancre.
 *
...
function melangeScores(t) {
  ...
  for (const m of cfg.METRIQUES) {
    ...
    // percentile — une seule ligne notee — l'ancre suffit, et c'est tout
    // l'interet d'une echelle absolue.
    if (absv === null) { t.score_metrique[cle] = relDispo ? rel : null; continue; }
    if (!relDispo || cfg.MELANGE_RELATIF === 0) { t.score_metrique[cle] = absv; continue; }
    t.score_metrique[cle] = cfg.MELANGE_RELATIF * rel + cfg.MELANGE_ABSOLU * absv;
  }
}
 ...
 */
/**
 * Piliers, total et couverture.
 *
 * `horsPortee` nomme les metriques qu'aucun titre de l'univers ne porte. Une
 * colonne absente de tout l'univers n'est pas une donnee manquante : c'est une
 * colonne que cette source ne produit pas. La compter comme un trou punissait
 * chaque societe pour la meme absence — les deux metriques previsionnelles
 * valent huit pour cent du poids total, et FinScope n'a pas de previsions et
 * n'en aura pas — ce qui plafonnait la couverture native sous le seuil et
...
function calculPiliers(t, clePct, poidsPiliers, horsPortee = new Set()) {
  ...
  for (const [pilier, metrs] of Object.entries(metrParPilier)) {
    ...
    for (const mm of metrs) {
      const w = mm.poids;
      if (horsPortee.has(mm.cle)) continue;
...
export function calculerScores(titres, poidsPiliers, winsoriser = true, horsPorteeDeclare = null) {
  ...
  //
  // Ce que personne ne porte n'est pas un trou : c'est hors de portee de la
  // source. Determine une fois pour l'univers, avant de noter quiconque.
  const horsPortee = horsPorteeDeclare
    ? new Set(horsPorteeDeclare)
    : new Set(
      cfg.METRIQUES
        .filter((m) => titres.every((t) => t.brut?.[m.cle] === null || t.brut?.[m.cle] === undefined))
        .map((m) => m.cle),
    );
