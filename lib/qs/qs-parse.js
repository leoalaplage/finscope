...
export function chargerTableau(texte, options = {}) {
  ...
  // Hors de portee : la source ne produit pas cette colonne du tout. C'est une
  // propriete de l'en-tete, pas du nombre de lignes — sans quoi noter une
  // societe seule effacerait chacun de ses trous et lui donnerait cent pour
  // cent de couverture. Une colonne presente mais vide reste un trou, et une
  // colonne absente qu'on sait reconstituer (OCF/Capex, les CAGR par action)
  // n'est pas hors de portee des lors qu'une ligne la porte.
  const horsPortee = cfg.METRIQUES
    .filter((m) => mapping[m.cle] === null && titres.every((t) => t.brut[m.cle] === null))
    .map((m) => m.cle);
  ...
  return { titres, manquantes, horsPortee, avertissements };
}
