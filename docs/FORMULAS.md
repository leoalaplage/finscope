...
| **Invested capital** | Total debt + Total equity − Cash and equivalents | The *financing* view; the operating build-up would need fixed assets and intangibles that several filers omit. **All three balances are required.** Total debt is the most complete filed reading at the balance-sheet date: an all-debt total where one exists, else the sum of non-overlapping current, long-term and short-term balances, else a single filed long-term balance on its own, which names what it excludes. Never two concepts naming the same balance, and never a long-term figure plus a current line that already sits inside it. Where `LongTermDebt` is ambiguous, it is treated as non-current only if the independently filed gross debt carrying amount reconciles to current + long-term within 3% and beats the competing interpretation by at least 2 percentage points; weak or conflicting evidence remains unavailable. A filed zero is a fact and is accepted. A filer whose complete borrowings this adapter still cannot read has no invested capital and therefore no ROIC and no cash return on capital, rather than a return flattered by a capital base missing its debt. |
...
| Metric | Formula | Rule |
|---|---|---|
| Market capitalization | Matched stock close × reported point-in-time shares outstanding | The period-end balance-sheet count is preferred. If it is absent, the same filing's cover-page count is used on the exact date the filer states; it is never redated as a closing balance. **The price must be quoted in the currency the statements are filed in**; nothing is converted, so for a filer reporting in another currency the market capitalisation and every multiple built on it are withheld with that reason stated. Where neither point-in-time count is readable — a multi-class filer may tag each class separately while only undimensioned facts reach us — the diluted weighted average stands in and the substitution is stated on the figure, never silent. |
...
| Shares outstanding | `us-gaap:CommonStockSharesOutstanding`, then `dei:EntityCommonStockSharesOutstanding` | The first is the balance-sheet parenthetical, dated at the period end, and always leads. The second is the count stated on the cover page — Apple's is 17 October against a 27 September year end. It is accepted only from the same filing accession, only between period end and filing date, and no more than 150 days after the close. Its actual observation date stays on the fact and every market-capitalisation use identifies the cover-date basis. A 110-filer sweep moved readable point-in-time counts from 62/86 to 77/86 normalized companies without borrowing a later filing's count for an older period. |
| Total debt | Filed all-debt total; otherwise compatible current + non-current long-term portions; otherwise explicitly non-overlapping long-term + short-term borrowings (+ finance leases where separately filed) | The current and non-current portions are two halves of one balance, not alternatives. JPMorgan's long-term aggregate is not its total borrowing because it separately reports short-term borrowings; both are required. A lone component never becomes total debt. |
