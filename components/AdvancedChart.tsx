...
import { cagrBetweenDates, convertUnit } from "@/lib/finance";
...
function rawSeriesValue(periods: FinancialPeriod[], index: number, metric: string, mode: ChartMode, anomalyMode: AnomalyMode) {
  ...
  let value = validatedDerivedValue(period, baseMetric, anomalyMode);
  ...
  if (growthMetric) { const prior = periods[index - 1]; const priorValue = prior ? validatedDerivedValue(prior, baseMetric, anomalyMode) : null; value = value != null && priorValue ? value / priorValue - 1 : null; }
  if (cagrMetric) value = index ? cagrBetweenDates(validatedDerivedValue(periods[0], baseMetric, anomalyMode), value, periods[0].periodEnd, period.periodEnd).value : null;
  return value;
}
...
export function AdvancedChart({ periods, metrics, unit, currency, mode, title, company = "Current company" }: { periods: FinancialPeriod[]; metrics: string[]; unit: Unit; currency: string; mode: ChartMode; title: string; company?: string }) {
  ...
  const data = useMemo(() => visiblePeriods.map((period, periodIndex) => {
    const values: Record<string, unknown> = { date: period.periodEnd, period };
    for (const item of series) { const raw = rawSeriesValue(visiblePeriods, periodIndex, item.metric, mode, anomalyMode); values[item.metric] = raw == null ? null : isPercent(item.metric, mode) ? raw * 100 : METRICS[item.metric]?.kind === "perShare" ? raw : convertUnit(raw, unit); }
    return values;
  }), [visiblePeriods, series, unit, mode, anomalyMode]);
