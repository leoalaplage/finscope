...
function ValuationView({ latest, company }: { latest: FinancialPeriod; company: CompanyDataset["company"] }) {
  ...
  useEffect(() => { let active=true; queueMicrotask(()=>{if(active){setLoading(true);setError("");setPrice(null)}}); fetch(`/api/price/${company.ticker}?date=${latest.periodEnd}`,{cache:"no-store"}).then(async (response) => { const payload=await response.json() as PricePoint & { error?: string }; if(!response.ok) throw new Error(payload.error); if(active)setPrice(payload); }).catch((cause) => active&&setError(cause instanceof Error?cause.message:"Price unavailable")).finally(() => active&&setLoading(false)); return()=>{active=false}; },[company.ticker,latest.periodEnd]);
