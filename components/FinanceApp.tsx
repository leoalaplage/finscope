...
function CompaniesPage({ watchlist, datasets, activeTicker, loading, onSearchAdd, onLoad, onOpen, onCharts, onRemove }: { watchlist: CompanyProfile[]; datasets: Record<string, CompanyDataset>; activeTicker: string; loading: string; onSearchAdd: () => void; onLoad: (ticker: string) => Promise<CompanyDataset>; onOpen: (ticker: string) => void; onCharts: (ticker: string) => void; onRemove: (ticker: string) => void }) {
  ...
  return <div><header className="page-heading"><div><h1>Watchlist</h1><p>{watchlist.length} {watchlist.length === 1 ? "company" : "companies"}, kept on this device · sorted by {(COMPANY_COLUMNS.find((column) => column.key === sort.key)?.label ?? "ticker").toLowerCase()}, {sort.direction === "desc" ? "highest first" : "lowest first"}.</p>{bulkState.total > 0 && <small>{bulkState.running ? `Loading all companies: ${bulkState.done}/${bulkState.total}` : `Load all finished: ${bulkState.done - bulkState.failed} loaded${bulkState.failed ? `, ${bulkState.failed} failed` : ""}`}</small>}</div><div className="company-title-actions"><button disabled={bulkState.running} onClick={() => void loadAll()}>{bulkState.running ? `Loading ${bulkState.done}/${bulkState.total}…` : "Load all"}</button><button onClick={onSearchAdd}>Add company</button></div></header>
    ...
    {/* The one picture on this page, and it used to open closed. A watchlist
        read as a table is forty numbers to compare by eye; the same companies
        plotted are one shape, and the outliers name themselves. */}
    <section className="scatter-panel"><div className="section-heading"><h2>Quality against price</h2><p className="section-note">{rows.length} {rows.length === 1 ? "company" : "companies"} · cash return against what the market charges for it. Click a point to open the company.</p></div>
