...
function Overview({ dataset, periods, unit, mode, onView }: { dataset: CompanyDataset; periods: FinancialPeriod[]; unit: Unit; mode: ChartMode; onView: (view: ViewKey) => void }) {
  ...
  return <>
    ...
    <AdvancedChart periods={periods} metrics={effectiveModeMetrics} unit={unit} currency={dataset.company.currency} mode={mode} title="FCF compounding & shareholder impact"/>
  </>;
}
...
function StatementView({ view, periods, unit, mode, currency, periodicity, onFact, onExport, onCopy }: { view: keyof typeof VIEW_METRICS; periods: FinancialPeriod[]; unit: Unit; mode: ChartMode; currency: string; periodicity: Periodicity; onFact: (fact: NormalizedFact) => void; onExport: (metrics: string[]) => void; onCopy: () => void }) {
...
function ValuationView({ latest, company }: { latest: FinancialPeriod; company: CompanyDataset["company"] }) {
  const [price,setPrice] = useState<PricePoint|null>(null); const [error,setError] = useState(""); const [loading,setLoading] = useState(true);
  useEffect(() => { let active=true; fetch(`/api/price/${company.ticker}?date=${latest.periodEnd}`).then(async (response) => { const payload=await response.json() as PricePoint & { error?: string }; if(!response.ok) throw new Error(payload.error); if(active)setPrice(payload); }).catch((cause) => active&&setError(cause instanceof Error?cause.message:"Price unavailable")).finally(() => active&&setLoading(false)); return()=>{active=false}; },[company.ticker,latest.periodEnd]);
  const metrics=valuationMetrics(latest,price); const cards=[
    ["Market capitalization",metrics?.marketCap,"currency"],["Price / Sales",metrics?.priceToSales,"multiple"],["Price / Earnings",metrics?.priceToEarnings,"multiple"],["Price / FCF",metrics?.priceToFreeCashFlow,"multiple"],["FCF yield",metrics?.freeCashFlowYield,"percent"],["Net buyback yield",metrics?.netBuybackYield,"percent"],
  ] as const;
  return <><section className="view-title"><div><span className="panel-kicker">HISTORICALLY MATCHED VALUATION</span><h2>Valuation at fiscal period end</h2><p>A current quote is never substituted for a missing historical session.</p></div></section>{loading ? <div className="notice"><Loader2 className="spin" size={18}/><div><b>Resolving Yahoo trading session</b><p>Checking the fiscal date, then preceding sessions within seven calendar days.</p></div></div> : price ? <div className="price-proof"><div><span>Fiscal period end</span><strong>{price.requestedDate}</strong></div><div><span>Price date used</span><strong>{price.date}</strong></div><div><span>Price type</span><strong>{price.type}</strong></div><div><span>Price</span><strong>{new Intl.NumberFormat("en-US",{style:"currency",currency:price.currency}).format(price.close)}</strong></div><div><span>Yahoo ticker</span><strong>{price.ticker}</strong></div><div><span>Fallback</span><strong>{price.fallback}</strong></div><a href={price.sourceUrl} target="_blank" rel="noreferrer">Source <ExternalLink size={12}/></a></div> : <div className="notice"><Info size={18}/><div><b>Yahoo Finance price unavailable</b><p>{error}</p></div><span>No substitution</span></div>}<section className="valuation-grid">{cards.map(([label,value,kind]) => <article className="valuation-card" key={label}><span>{label}</span><strong>{value == null ? "—" : kind === "currency" ? compactCurrency(value,company.currency) : kind === "percent" ? `${(value*100).toFixed(2)}%` : `${value.toFixed(1)}×`}</strong><small>{value == null ? "Unavailable" : `Based on ${price?.date}`}</small></article>)}</section><section className="panel methodology-card"><div><span className="panel-kicker">PRICE MATCHING RULE</span><h2>Weekend and holiday safe</h2><p>Exact date first, then the last prior session within seven days, then a clearly flagged next-session fallback within two days. Adjusted close is preferred because Yahoo defines it as split- and dividend-adjusted.</p></div><div className="formula-box"><code>Market cap = matched price × diluted shares</code><code>P/FCF = market cap / free cash flow</code><code>Net buyback yield = net repurchases / market cap</code></div></section></>;
}
