...
export function HomeWatchlist() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(`/api/watchlist?tickers=${encodeURIComponent(unknown)}&v=${KEY_VERSION}.${SUMMARY_SHAPE}`, { signal: controller.signal });
  ...
  return (
    <section className="quick" aria-labelledby="watchlist-title">
      <div className="quick-head">
        ...
        <button className="watchlist-edit" type="button" onClick={() => setEditing(true)} aria-label="Edit watchlist" title="Edit watchlist"><Pencil size={11} /></button>
      </div>
      ...
      {editing ? (
        <WatchlistEditor tickers={tickers} onClose={() => setEditing(false)} onSaved={setSessionTickers} />
      ) : null}
    </section>
  );
}
